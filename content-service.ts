...
export class ContentService {
   ...
   * 更新内容状态
   */
  updateContentStatus(contentId: ID, newStatus: ContentStatus): Content | undefined {
    const content = this.store.getContent(contentId);
    if (!content) return undefined;
    
    const validStatuses: ContentStatus[] = ['draft', 'published', 'archived', 'deleted'];
   ...
   * 发布草稿
   */
  publishDraft(contentId: ID): Content | undefined {
   ...
   * 归档内容
   */
  archiveContent(contentId: ID): Content | undefined {
   ...
   * 软删除内容
   */
  softDelete(contentId: ID): Content | undefined {
    const content = this.store.getContent(contentId);
    if (!content) return undefined;
   ...
   * 恢复已删除内容
   */
  restoreContent(contentId: ID): Content | undefined {
    const content = this.store.getContent(contentId);
    if (!content) return undefined;
   ...
   */
  getTrash(): Content[] {
    return this.store.listContents({ includeDeleted: true })
   ...
   * 设置父子关系（如文章与章节）
   */
  setParentChild(parentId: ID, childId: ID, order?: number): boolean {
   ...
   * 获取内容的子内容
   */
  getChildren(contentId: ID): Content[] {
    const content = this.store.getContent(contentId);
    if (!content) return [];
   ...
   * 获取内容的父内容
   */
  getParent(contentId: ID): Content | undefined {
    const content = this.store.getContent(contentId);
    if (!content) return undefined;
  ...
  addReference(
    fromId: ID, 
    toId: ID, 
    relationType: ReferenceRelationType = 'link'
  ): boolean {
   ...
   * 获取引用了某内容的所有内容
   */
  getReferringTo(contentId: ID): Content[] {
   ...
   * 获取某内容引用的所有内容
   */
  getReferencesFrom(contentId: ID): Content[] {
    const content = this.store.getContent(contentId);
    if (!content) return [];
   ...
   * 添加点赞
   */
  addLike(contentId: ID, author: Author): boolean {
    const content = this.store.getContent(contentId);
   ...
   * 获取内容的所有互动
   */
  getInteractions(contentId: ID, type?: InteractionType): Interaction[] {
    const content = this.store.getContent(contentId);
    if (!content) return [];
   ...
   * 获取内容的完整信息（包含关系）
   */
  getContentWithRelations(contentId: ID): {
    content: Content;
    parent?: Content;
    children: Content[];
    references: Content[];
  ...
  createCommunity(name: string, description?: string, founderId?: ID): Community {
  ...
  joinCommunity(memberId: ID, communityId: ID): boolean {
    const member = this.store.getMember(memberId);
    const community = this.store.getCommunity(communityId);
