...
function createApiServer(service) {
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      // ===== 社区管理 =====
      if (pathname === '/communities' && method === 'POST') {
        const body = await parseBody(req);
      ...
      if (pathname === '/communities' && method === 'GET') {
        const communities = service.store.listCommunities();
        sendJson(res, communities);
        return;
      }
      
      if (pathname.startsWith('/communities/') && pathname.endsWith('/join') && method === 'POST') {
      ...
      // ===== 内容管理 =====
      if (pathname === '/contents' && method === 'POST') {
        const body = await parseBody(req);
      ...
      if (pathname.startsWith('/contents/') && method === 'GET') {
      ...
      // ===== 结构关系 =====
      if (pathname.startsWith('/contents/') && pathname.endsWith('/children') && method === 'GET') {
      ...
      if (pathname === '/relations/structural' && method === 'POST') {
        const body = await parseBody(req);
      ...
      // ===== 引用关系 =====
      if (pathname === '/relations/reference' && method === 'POST') {
        const body = await parseBody(req);
      ...
      // ===== 互动 =====
      if (pathname.startsWith('/contents/') && pathname.endsWith('/like') && method === 'POST') {
        const contentId = pathname.split('/')[2];
        const body = await parseBody(req);
      ...
      if (pathname.startsWith('/contents/') && pathname.endsWith('/comment') && method === 'POST') {
        const contentId = pathname.split('/')[2];
        const body = await parseBody(req);
      ...
      // 回复内容
      if (pathname.startsWith('/contents/') && pathname.endsWith('/reply') && method === 'POST') {
        const contentId = pathname.split('/')[2];
        const body = await parseBody(req);
        try {
      ...
      // ===== 内容状态管理 =====
      if (pathname.startsWith('/contents/') && pathname.endsWith('/status') && method === 'PUT') {
        const contentId = pathname.split('/')[2];
        const body = await parseBody(req);
        try {
      ...
      if (pathname.startsWith('/contents/') && pathname.endsWith('/publish') && method === 'POST') {
        const contentId = pathname.split('/')[2];
      ...
      if (pathname.startsWith('/contents/') && pathname.endsWith('/archive') && method === 'POST') {
        const contentId = pathname.split('/')[2];
      ...
      // ===== 回收站功能 =====
      if (pathname.startsWith('/contents/') && method === 'DELETE') {
        const parts = pathname.split('/');
        if (parts.length === 3) {
          const contentId = parts[2];
      ...
      if (pathname.startsWith('/contents/') && pathname.endsWith('/restore') && method === 'POST') {
        const contentId = pathname.split('/')[2];
